...
import { SafeAreaView, Text, StyleSheet, FlatList, Pressable, Modal, TextInput, Alert, KeyboardAvoidingView, Platform, ActivityIndicator, View, Dimensions, Image, ScrollView, TouchableOpacity, Animated, Easing, Keyboard } from "react-native";
import AsyncStorage from "@react-native-async-storage/async-storage";
import Ionicons from "react-native-vector-icons/Ionicons";
import BottomNavBar from "./components/BottomNavbar";
import { fetchConversations, getMessages } from "./api";
import { getFirestore, doc, onSnapshot, setDoc, updateDoc, arrayUnion, collection, query, where, documentId } from "firebase/firestore";
...
type Request = { requestId: string; productId: string; productTitle: string; buyerId: string; sellerId: string; status: string; createdAt: string; buyerFirstName: string; buyerLastName: string; sellerFirstName: string; sellerLastName: string; referenceType?: string; referenceTitle?: string; };
...
const MessagingScreen: React.FC<MessagingScreenProps> = ({ route }) => {
  ...
  const reasons = ["Inappropriate Behavior", "Fraudulent Activity", "Harassment", "Scamming", "Other"];
  const CLOUDINARY_URL = "https://api.cloudinary.com/v1_1/ds0zpfht9/image/upload";
  const UPLOAD_PRESET = "gridly_preset";
  const applyFilter = (chatsToFilter: Chat[], f: "all" | "products" | "gigs" | "product_request") => {
    if (f === "all") return chatsToFilter;
    if (f === "products") return chatsToFilter.filter((c) => c.referenceType === "product");
    if (f === "gigs") return chatsToFilter.filter((c) => c.referenceType === "gig");
        ...
        if (!docSnap.exists()) {
          ...
          if (data.messages) {
            setSelectedChat((prev) => {
              if (!prev) return { ...selectedChat, messages: data.messages, sold: data.sold };
              if (!prev.messages || !Array.isArray(data.messages)) return prev;
              if (data.messages.length <= prev.messages.length) return { ...prev, sold: data.sold };
              return { ...prev, messages: data.messages, sold: data.sold };
            });
            setTimeout(() => { flatListRef.current?.scrollToEnd({ animated: true }); }, 100);
          } else if (data.sold !== undefined) {
            setSelectedChat((prev) => prev ? { ...prev, sold: data.sold } : prev);
          }
        }
      });
    }
  ...
  useEffect(() => {
    if (chats.length > 0) {
      ...
      const q = query(collection(firestoreDB, "chatRooms"), where(documentId(), "in", chatIDs));
  ...
  const handleBackFromChat = async () => {
    if (selectedChat) {
      ...
      setChats((prevChats) =>
        [...prevChats.map((c) => (c.chatID === updatedChat.chatID ? updatedChat : c))].sort((a, b) => {
          const aTime = a.latestTimestamp ? new Date(a.latestTimestamp).getTime() : 0;
          const bTime = b.latestTimestamp ? new Date(b.latestTimestamp).getTime() : 0;
          return bTime - aTime;
        })
  ...
  const sendImageMessage = async (imageUrl: string) => {
    ...
    const chatDocRef = doc(firestoreDB, "chatRooms", selectedChat.chatID);
    await setDoc(chatDocRef, { messages: arrayUnion(msgData) }, { merge: true });
  };
  const uploadImageToCloudinary = async (uri: string): Promise<string> => {
    ...
    try {
      ...
      formDataImage.append("file", { uri: manipulatedImage.uri, type: "image/jpeg", name: `upload_${Date.now()}.jpg` } as any);
      formDataImage.append("upload_preset", UPLOAD_PRESET);
      const response = await axios.post(CLOUDINARY_URL, formDataImage, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const fetchUserRequests = async () => {
    ...
    try {
      const response = await axios.get("https://thegridproduct-production.up.railway.app/chat/request", { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
      if (response.status === 200) {
        ...
        setHasNewIncomingRequests(incomingRequests && incomingRequests.length > 0);
      } else {
        setErrorRequests("Failed to fetch chat requests.");
      }
    } catch (error: any) {
      console.error("Error fetching requests:", error);
      setErrorRequests(error.response?.data?.message || "An error occurred while fetching chat requests.");
    } finally {
      setLoadingRequests(false);
    }
  };
  const acceptRequest = async (requestId: string) => {
    ...
    try {
      const response = await axios.post("https://thegridproduct-production.up.railway.app/chat/accept", { requestId }, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
      if (response.status === 200) {
  ...
  const rejectRequest = async (requestId: string) => {
    setProcessingRequestId(requestId);
    try {
      const response = await axios.post("https://thegridproduct-production.up.railway.app/chat/reject", { requestId }, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
      if (response.status === 200) {
  ...
  const renderChat = ({ item }: { item: Chat }) => {
    ...
    const latestMessage = item.latestMessage ? { content: item.latestMessage, timestamp: item.latestTimestamp, senderId: item.latestSenderId } : null;
    const formattedTimestamp = latestMessage ? new Date(latestMessage.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "";
    const unreadCount = unreadCounts[item.chatID] || 0;
    const chatTypeLabel = item.referenceType === "product" ? "Product Name: " : item.referenceType === "gig" ? "Gig Name: " : item.referenceType === "product_request" ? "Product Request Name: " : "Unnamed Item";
    return (
      <Pressable style={styles.chatItemContainer} onPress={() => openChat(item)} accessibilityLabel={`Open chat with ${item.user.firstName} ${item.user.lastName}`}>
          ...
          <View style={styles.chatDetails}>
            ...
            {latestMessage && (
              <View style={styles.lastMessageRow}>
                <View style={latestMessage.senderId !== userId ? styles.purpleDot : styles.greyDot} />
                <Text style={styles.lastMessage} numberOfLines={1}>
                  {latestMessage.content.startsWith("[Image] ") ? "Image" : latestMessage.content}
                </Text>
              </View>
            )}
          </View>
      ...
      <View style={[styles.messageContainer, isCurrentUser ? styles.myMessageContainer : styles.theirMessageContainer]}>
        <View style={[styles.messageBubble, isCurrentUser ? styles.myMessage : styles.theirMessage]}>
          {isImageMessage ? (
            ...
            <Text style={[styles.messageText, isCurrentUser ? styles.myMessageTextColor : styles.theirMessageTextColor]}>
              {item.content}
            </Text>
          )}
          <Text style={[styles.messageTimestamp, isCurrentUser ? styles.myTimestampColor : styles.theirTimestampColor]}>
  ...
  const handleDeleteChat = async () => {
    ...
    Alert.alert("Confirm Delete", "Are you sure you want to delete this chat?", [
      ...
      { text: "Delete", style: "destructive", onPress: async () => {
          try {
            const response = await axios.delete(`https://thegridproduct-production.up.railway.app/chats/${selectedChat.chatID}`, { headers: { Authorization: `Bearer ${token}` } });
            if (response.status === 200) {
              setChats((prevChats) => prevChats.filter((c) => c.chatID !== selectedChat.chatID));
              setChatModalVisible(false);
              setSelectedChat(null);
            ...
            Alert.alert("Error", error.response?.data?.message || "Failed to delete chat.");
          }
        }
      },
    ]);
  ...
  const handleMarkAsSold = async () => {
    if (!selectedChat) return;
    Alert.alert("Confirm", "Have you sold the product? It will be removed from the marketplace.", [
      { text: "Cancel", style: "cancel" },
      { text: "OK", onPress: async () => {
          try {
            const response = await axios.put(`https://thegridproduct-production.up.railway.app/chats/${selectedChat.chatID}/complete`, {}, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
            if (response.status === 200) {
              const chatDocRef = doc(firestoreDB, "chatRooms", selectedChat.chatID);
            ...
            Alert.alert("Error", error.response?.data?.message || "Failed to update status of the item.");
          }
        }
      },
    ]);
  ...
  const currentHeaderTitle = filter === "all" ? "All Chats" : filter === "products" ? "Chats" : filter === "gigs" ? "Chats" : filter === "product_request" ? "Chats" : "All Chats";
  const currentFilterLabel = filter === "all" ? "All ×" : filter === "products" ? "Products ×" : filter === "gigs" ? "Jobs ×" : filter === "product_request" ? "Product Requests ×" : "All ×";
  const handleFilterPillPress = () => { setFilterMenuVisible(true); };
  const handleReportPress = () => { setReportModalVisible(true); setChatModalVisible(false); };
  const fetchChatDetails = async (chatId: string) => {
    ...
    try {
      const response = await axios.get(`${NGROK_URL}/chats/${chatId}`, { headers: { Authorization: `Bearer ${token}` } });
      ...
      console.error("Error fetching chat details:", error.response?.data || error);
      return null;
    }
  };
  const handleModifiedReportSubmit = async () => {
    if (reportReason === "Select a Reason" || !reportDescription.trim() || (reportReason === "Other" && !customReason.trim())) {
    ...
    try {
      const response = await axios.post(`${NGROK_URL}/report`, { chatId: selectedChat.chatID, reason: reportReason === "Other" ? customReason.trim() : reportReason, description: reportDescription.trim() }, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
      if (response.status === 200) {
  ...
  const renderRequestsModal = () => {
    if (isRequestsModalVisible) {
      Animated.timing(slideAnim, { toValue: 0, duration: 250, easing: Easing.inOut(Easing.ease), useNativeDriver: true }).start();
    } else {
      Animated.timing(slideAnim, { toValue: Dimensions.get("window").height, duration: 250, easing: Easing.inOut(Easing.ease), useNativeDriver: true }).start();
    }
    const renderRequestItem = ({ item }: { item: Request }) => {
      const referenceTypeLabel = item.referenceType === "product" ? "Product Name: " : item.referenceType === "gig" ? "Gig Name: " : item.referenceType === "product_request" ? "Product Request Name: " : "Unnamed Item";
      return (
        <View style={styles.requestCard}>
          <View style={styles.requestInfo}>
            <Text style={styles.requestProductName}>{referenceTypeLabel} {item.referenceTitle || "Unnamed Item"}</Text>
            <Text style={styles.requestSender}>{selectedRequestsTab === "incoming" ? `From: ${item.buyerFirstName} ${item.buyerLastName}` : `To: ${item.sellerFirstName} ${item.sellerLastName}`}</Text>
            <Text style={styles.requestDate}>Created on: {new Date(item.createdAt).toLocaleDateString()}</Text>
          </View>
          {selectedRequestsTab === "incoming" ? (
            <View style={styles.requestActions}>
              <TouchableOpacity style={[styles.acceptButton, processingRequestId === item.requestId && styles.buttonDisabled]} onPress={() => acceptRequest(item.requestId)} disabled={processingRequestId === item.requestId} accessibilityLabel="Accept Request">
                {processingRequestId === item.requestId ? <ActivityIndicator color="#fff" size="small" /> : <Ionicons name="checkmark" size={20} color="#fff" />}
              </TouchableOpacity>
              <TouchableOpacity style={[styles.rejectButton, processingRequestId === item.requestId && styles.buttonDisabled]} onPress={() => rejectRequest(item.requestId)} disabled={processingRequestId === item.requestId} accessibilityLabel="Reject Request">
                {processingRequestId === item.requestId ? <ActivityIndicator color="#fff" size="small" /> : <Ionicons name="close" size={20} color="#fff" />}
              </TouchableOpacity>
            </View>
          ) : (
            <View style={styles.requestActions} />
    ...
    return (
      <Modal visible={isRequestsModalVisible} animationType="none" transparent={true} onRequestClose={() => setRequestsModalVisible(false)}>
        <Animated.View style={[styles.requestsModalContainer, { transform: [{ translateY: slideAnim }] }]}>
          <SafeAreaView style={styles.requestsSafeArea}>
            ...
            <View style={styles.requestsTabsRow}>
              <Pressable onPress={() => setSelectedRequestsTab("incoming")} style={[styles.requestsTab, selectedRequestsTab === "incoming" && styles.activeRequestsTab]} accessibilityLabel="Incoming Requests Tab">
                <Text style={[styles.requestsTabText, selectedRequestsTab === "incoming" && styles.activeRequestsTabText]}>Incoming</Text>
              </Pressable>
              <Pressable onPress={() => setSelectedRequestsTab("outgoing")} style={[styles.requestsTab, selectedRequestsTab === "outgoing" && styles.activeRequestsTab]} accessibilityLabel="Outgoing Requests Tab">
                <Text style={[styles.requestsTabText, selectedRequestsTab === "outgoing" && styles.activeRequestsTabText]}>Outgoing</Text>
              </Pressable>
            </View>
            {loadingRequests ? (
              ...
              <View style={styles.requestsContent}>
                <FlatList data={dataToShow} keyExtractor={(item) => item.requestId} renderItem={renderRequestItem} ListEmptyComponent={
                  <View style={styles.sectionEmptyContainer}>
                    <Text style={styles.sectionEmptyText}>{selectedRequestsTab === "incoming" ? "No incoming requests." : "No outgoing requests."}</Text>
                  </View>
                } ItemSeparatorComponent={() => (<View style={styles.requestsSeparatorLine} />)} refreshing={loadingRequests} onRefresh={fetchUserRequests} />
              </View>
            )}
          </SafeAreaView>
        </Animated.View>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerButtons}>
            <Pressable style={styles.requestsButton} onPress={() => { setRequestsModalVisible(true); setSelectedRequestsTab("incoming"); fetchUserRequests(); }} accessibilityLabel="View Requests">
              <View style={{ position: "relative" }}>
                <Ionicons name="list-circle-outline" size={24} color="#BB86FC" />
                {hasNewIncomingRequests && (<View style={styles.exclamationBadge}><Ionicons name="alert" size={12} color="#fff" /></View>)}
              </View>
        ...
        <Modal visible={filterMenuVisible} animationType="fade" transparent={true} onRequestClose={() => setFilterMenuVisible(false)}>
          <View style={styles.modalOverlay}>
            <View style={styles.filterModalContainer}>
              <Pressable style={styles.filterModalOption} onPress={() => { setFilter("all"); setFilterMenuVisible(false); setFilteredChats(applyFilter(chats, "all")); }}>
                <Text style={styles.filterModalOptionText}>All</Text>
              </Pressable>
              <Pressable style={styles.filterModalOption} onPress={() => { setFilter("products"); setFilterMenuVisible(false); setFilteredChats(applyFilter(chats, "products")); }}>
                <Text style={styles.filterModalOptionText}>Products</Text>
              </Pressable>
              <Pressable style={styles.filterModalOption} onPress={() => { setFilter("gigs"); setFilterMenuVisible(false); setFilteredChats(applyFilter(chats, "gigs")); }}>
                <Text style={styles.filterModalOptionText}>Jobs</Text>
              </Pressable>
              <Pressable style={styles.filterModalOption} onPress={() => { setFilter("product_request"); setFilterMenuVisible(false); setFilteredChats(applyFilter(chats, "product_request")); }}>
                <Text style={styles.filterModalOptionText}>Product Requests</Text>
              </Pressable>
              <Pressable style={[styles.filterModalOption, styles.filterModalClose]} onPress={() => setFilterMenuVisible(false)}>
          ...
          <View style={styles.horizontalListContainer}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
              {productsOrGigs.map((item, index) => (
                <Pressable key={item.chatID + index} style={styles.productGigItem} onPress={() => handleNavigateFromProductOrGig(item)} accessibilityLabel={`Navigate to ${item.type} titled ${item.title}`}>
                  <Text style={styles.productGigItemText}>{item.type === "product" ? "Product: " : "Gig: "} {item.title}</Text>
                </Pressable>
              ))}
            </ScrollView>
          </View>
        ...
        {loading && chats.length === 0 ? (
          ...
          <FlatList style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 60 }} data={filteredChats} keyExtractor={(item, index) => item.chatID || index.toString()} renderItem={renderChat} ItemSeparatorComponent={() => <View style={styles.separatorLine} />} />
        )}
          ...
          <View style={[styles.modalSafeArea, { backgroundColor: "#000" }]}>
            <SafeAreaView style={{ flex: 1 }}>
              <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.modalContainer}>
                  ...
                  <View style={styles.enhancedChatHeader}>
                    ...
                    <View style={styles.chatHeaderInfo}>
                      ...
                      <View style={styles.chatHeaderTextContainer}>
                        ...
                        <Text style={styles.chatHeaderSubTitle}>
                          {selectedChat?.referenceType ? selectedChat.referenceType.charAt(0).toUpperCase() + selectedChat.referenceType.slice(1) : ""}
                          {selectedChat?.referenceTitle ? `: ${selectedChat.referenceTitle}` : ""}
                        </Text>
                      </View>
                    </View>
                    ...
                    {selectedChat?.referenceType === "product" && (
                      selectedChat.sold ? (
                        ...
                        <View style={styles.productActionButtons}>
                          <Pressable onPress={handleDeleteChat} style={[styles.smallActionButton, styles.smallActionButtonClose]} accessibilityLabel="Close Chat">
                            <Ionicons name="close" size={16} color="#FFFFFF" />
                          </Pressable>
                          <Pressable onPress={handleMarkAsSold} style={[styles.smallActionButton, styles.smallActionButtonCheck]} accessibilityLabel="Mark as Sold">
                ...
                <FlatList ref={flatListRef} data={selectedChat?.messages || []} keyExtractor={(item, index) => item._id || index.toString()} renderItem={renderMessage} contentContainerStyle={styles.messagesList} onContentSizeChange={() => flatListRef.current?.scrollToEnd({ animated: true })} onLayout={() => flatListRef.current?.scrollToEnd({ animated: true })} ListEmptyComponent={
                ...
                <View style={styles.inputBarContainer}>
                  ...
                  <View style={styles.inputContainer}>
                    ...
                    <TextInput style={styles.messageInput} placeholder="Type a message..." placeholderTextColor="#555" value={newMessage} onChangeText={setNewMessage} multiline accessibilityLabel="Message Input" blurOnSubmit onSubmitEditing={Keyboard.dismiss} />
        ...
        <Modal visible={isImagePreviewModalVisible} animationType="slide" transparent={false} onRequestClose={() => { setIsImagePreviewModalVisible(false); setSelectedImageUri(null); }}>
          <View style={[styles.modalSafeArea, { backgroundColor: "#000" }]}>
            <SafeAreaView style={{ flex: 1 }}>
              <View style={styles.enhancedChatHeader}>
                <Pressable onPress={() => { setIsImagePreviewModalVisible(false); setSelectedImageUri(null); }} style={styles.backButton} accessibilityLabel="Go Back">
                  <Ionicons name="arrow-back" size={24} color="#BB86FC" />
                </Pressable>
                <View style={{ flex: 1 }} />
                <Pressable onPress={confirmAddImage} style={styles.addImageButton} accessibilityLabel="Add Image" disabled={sending || isUploadingImage}>
                  {sending || isUploadingImage ? <ActivityIndicator color="#fff" size="small" /> : <Ionicons name="checkmark" size={20} color="#BB86FC" />}
                </Pressable>
              </View>
              {selectedImageUri && (
          ...
          <KeyboardAvoidingView style={{ flex: 1, backgroundColor: "#000" }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
            <SafeAreaView style={{ flex: 1, padding: 20 }}>
              ...
              <View style={styles.reportContent}>
                ...
                {isReasonDropdownVisible && (
                  <View style={styles.dropdownAbsoluteOverlay}>
                    <View style={styles.dropdownAbsoluteContainer}>
                      {reasons.map((reason) => (
                        <Pressable key={reason} style={styles.dropdownOption} onPress={() => { setReportReason(reason); setIsReasonDropdownVisible(false); }}>
                ...
                {reportReason === "Other" && (
                  <TextInput style={styles.otherReasonInput} placeholder="Enter your reason..." placeholderTextColor="#888" value={customReason} onChangeText={setCustomReason} blurOnSubmit onSubmitEditing={Keyboard.dismiss} />
                )}
                <Text style={styles.reportLabel}>Description:</Text>
                <TextInput style={styles.reportDescriptionInput} placeholder="Describe the issue..." placeholderTextColor="#888" value={reportDescription} onChangeText={setReportDescription} multiline blurOnSubmit onSubmitEditing={Keyboard.dismiss} />
                ...
                <Pressable style={styles.reportSubmitButton} onPress={handleModifiedReportSubmit} disabled={isSubmittingReport}>
                  {isSubmittingReport ? <ActivityIndicator color="#000" size="small" /> : <Text style={styles.reportSubmitButtonText}>Submit</Text>}
                </Pressable>
              </View>
            </SafeAreaView>
          </KeyboardAvoidingView>
        ...
        <Modal transparent visible={isIncompletePopupVisible} animationType="fade" onRequestClose={() => setIsIncompletePopupVisible(false)}>
        ...
        <Modal transparent visible={requestPopupVisible} animationType="fade" onRequestClose={() => setRequestPopupVisible(false)}>
          <View style={styles.popupOverlay}>
            <View style={styles.popupContainer}>
              <Text style={styles.popupText}>{requestPopupMessage}</Text>
              <Pressable style={styles.popupButton} onPress={() => setRequestPopupVisible(false)}>
...
const styles = StyleSheet.create({
  ...
  requestSender:{ fontSize:14, fontWeight:"bold", color:"#FFFFFF", fontFamily:"HelveticaNeue", marginBottom:5 },
  ...
  soldLabel:{ color:"#FFFFFF", fontSize:16, fontWeight:"bold", fontFamily:"HelveticaNeue", marginRight:10 }
});
