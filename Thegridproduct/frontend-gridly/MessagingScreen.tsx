...
const MessagingScreen: React.FC<MessagingScreenProps> = ({ route }) => {
  ...
  // Push Notification integration state
  ...
  // Request and retrieve the Expo push token
  useEffect(() => {
    (async () => {
  ...
  // Helper function to send a push notification via your backend.
  // Errors are caught and logged, so no error is shown to the user.
  const sendPushNotification = async (title: string, message: string) => {
    if (!expoPushToken) return;
    try {
      await fetch(`${NGROK_URL}/sendPushNotification`, {
        method: "POST",
  ...
  // Listen to changes in the currently open chat room.
  useEffect(() => {
        ...
        if (!docSnap.exists()) {
          ...
          if (data.messages) {
            setSelectedChat((prev) => {
              ...
              // When a new message arrives that is not from the current user, send a push notification.
              if (data.messages.length > prev.messages.length) {
                const newMsg = data.messages[data.messages.length - 1];
                if (newMsg.senderId !== userId) {
                  sendPushNotification("New Message", `New message from ${selectedChat.user.firstName}`);
                }
              }
  ...
  const renderRequestsModal = () => {
    ...
    return (
      <Modal visible={isRequestsModalVisible} animationType="none" transparent onRequestClose={() => setRequestsModalVisible(false)}>
  ...
  const handleReportPressWrapper = () => { setReportModalVisible(true); setChatModalVisible(false); };
  ...
  return (
    <View style={styles.container}>
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <Modal visible={filterMenuVisible} animationType="fade" transparent onRequestClose={() => setFilterMenuVisible(false)}>
        ...
        {chats.length > 0 && (
          <View style={styles.horizontalListContainer}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
              {chats.map((c, index) => (
                <Pressable key={c.chatID + index} style={styles.productGigItem} onPress={() => openChat(c)} accessibilityLabel={`Navigate to chat for ${c.referenceTitle}`}>
                  <Text style={styles.productGigItemText}>{c.referenceType === "product" ? "Product: " : c.referenceType === "gig" ? "Gig: " : "Item: "} {c.referenceTitle || "Unnamed Item"}</Text>
                </Pressable>
              ))}
            </ScrollView>
          </View>
              ...
              <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.modalContainer}>
                {selectedChat && (
                  <View style={styles.enhancedChatHeader}>
                    ...
                    <Pressable onPress={handleReportPressWrapper} style={styles.reportButton} accessibilityLabel="Report User">
                    ...
                    {selectedChat?.referenceType === "product" && (
                      selectedChat.sold ? (
                        ...
                        <View style={styles.productActionButtons}>
                          <Pressable onPress={handleBackFromChat} style={[styles.smallActionButton, styles.smallActionButtonClose]} accessibilityLabel="Close Chat">
                            <Ionicons name="close" size={16} color="#FFFFFF" />
                          </Pressable>
                          <Pressable onPress={handleBackFromChat} style={[styles.smallActionButton, styles.smallActionButtonCheck]} accessibilityLabel="Mark as Sold">
